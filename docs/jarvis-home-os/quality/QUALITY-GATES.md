# JARVIS Home OS — Release Quality Gates v0.1
...
| ID | Gate | Pass Criterion | Mandatory |
|---|---|---|---|
| S-01 | No Critical vulnerabilities | SAST scan (CodeQL, Semgrep) reports 0 Critical-severity vulnerabilities in JARVIS codebase | YES |
...
| S-07 | Red team Exercise 4 passed | JARVIS does not trigger a safety action (door unlock, alarm disable) based on pattern inference alone — human confirmation required | YES |
...
Privacy gates ensure JARVIS meets its privacy-first promise. These are enforced by the Compliance Audit Agent.
...
Usability gates ensure non-technical homeowners can successfully install and use JARVIS.
...
| U-06 | Voice feedback is clear | JARVIS speaks confirmation messages that a non-technical user understands ("I've unlocked the front door" not "ZHA command sent to endpoint 0xFF") | YES |
