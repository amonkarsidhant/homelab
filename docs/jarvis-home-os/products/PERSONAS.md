# JARVIS Home OS — User Personas v0.1
...
### How She'd Use JARVIS

**Day 1 onboarding:**
- "Hey JARVIS, find my devices"
- JARVIS discovers Hue bulbs and Nest thermostat
- Priya: "Organize them by room"
- Done — no app, no YAML, no configuring

**Daily interactions:**
- "Hey JARVIS, I'm going to work" → JARVIS locks doors, sets thermostat to away mode
- "Hey JARVIS, movie time" → JARVIS dims living room lights, turns off kitchen lights
- "Hey JARVIS, did the kids get home from school?" → Motion sensor check, notification
...
2. **Reliability trust:** JARVIS does what it says. If she says "lock the doors," the doors are locked. If it can't, it tells her immediately.
3. **Parental trust:** She needs to know her kids can't bypass safety settings by asking JARVIS nicely. No voice command should unlock a door if parental controls are on.
4. **Brand trust:** She doesn't know JARVIS as a startup. If IKEA or Wayfair offers it, she trusts it because she already trusts the store.
...
- Full control — he wants to be able to audit every decision JARVIS makes
...
**Day 1 onboarding:**
- Marcus manually configures all his Home Assistant devices into JARVIS via config file
- He enables debug logging immediately
- He reads the local inference source code to verify privacy claims
- He writes a custom skill to expose his ESPHome sensor data to JARVIS

**Daily interactions:**
- "Hey JARVIS, status" → List of all device states, recent events, any anomalies
- He uses JARVIS voice to trigger complex Home Assistant scripts via command line
- He reviews pattern learning outputs and manually approves/rejects them

**Advanced usage:**
- He forks the JARVIS skill SDK and builds a custom energy monitoring skill
- He contributes it back to the community after 3 months of internal use
- He attends the JARVIS developer forum to discuss architecture decisions

### What "Trust" Means to Him

1. **Source trust:** He will read the code. If he can't, he doesn't trust it. JARVIS must be open-source or have a verifiable open-source core.
...
3. **Control trust:** He can disable or override any behavior. JARVIS is his agent, not his replacement.
4. **Community trust:** If JARVIS has a thriving developer community, he trusts it more. Dead projects are untrustworthy.
...
### How They'd Use JARVIS

**Week 1 — Evaluation:**
- Jordan reviews JARVIS Enterprise documentation
- Schedules a technical call with JARVIS engineering to discuss white-label architecture
- Asks for a sandbox environment to demo internally

**Month 1-3 — Pilot:**
- JARVIS Enterprise provisions a sandbox for their internal lab
...
- JARVIS is re-skinned as "[Brand] Home" — same JARVIS brain, brand voice
...
1. **Enterprise trust:** JARVIS Enterprise must have SOC 2 Type II certification, GDPR compliance, and clear data handling terms before they can sign a contract.
2. **Brand safety trust:** JARVIS must never do anything that damages their brand — no inappropriate content, no anti-brand sentiment from the voice agent.
3. **Operational trust:** If JARVIS goes down, their customer support gets calls. They need SLA guarantees (99.9% uptime minimum) and clear escalation paths.
