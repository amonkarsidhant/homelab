...
JARVIS Home OS is designed for **self-hosted deployment** — a non-technical homeowner should be able to flash an SD card and have a working system within 30 minutes. This document defines the deployment model, Day 1 ship artifact, onboarding UX, update mechanism, and minimum hardware requirements.
...
**JARVIS Hub Image — Raspberry Pi Imager compatible**
...
│   │   ├── systemd/system/docker-compose@jarvis.service
...
│   ├── usr/local/bin/
│   │   ├── jarvis-setup (first-boot wizard CLI)
│   │   └── jarvis-update (update script)
│   └── docker/
│       ├── jarvis-core.tar (preloaded, loaded on first boot)
...
**Download:** Hosted on JARVIS Cloud (opt-in download) and IPFS (decentralized fallback).
...
```
1. Homeowner downloads "JARVIS Home OS" from jarvis.homeos.io
2. Opens Raspberry Pi Imager
3. Selects "JARVIS Home OS" custom image (discovers .img via API or manual download)
...
```
1. Pi boots, JARVIS setup wizard runs automatically
2. User connects phone to JARVIS WiFi ("JARVIS-XXXX" hotspot)
3. User opens http://setup.jarvis.local in browser
4. Setup wizard:
   ...
   d. Download JARVIS app from App Store / Play Store
   e. Scan QR code on Pi to link app to hub
5. Setup complete — "Hey JARVIS" is now active
```
...
1. Say "Hey JARVIS, turn on the living room lights" — lights turn on
2. Open the JARVIS app — see device dashboard
...
**Goal:** Make JARVIS appear as a first-class OS choice in Raspberry Pi Imager.
...
**Interim:** Host image on jarvis.homeos.io with manual download instructions.
...
```
┌─────────────────┐         ┌─────────────────┐         ┌─────────────────┐
│  JARVIS Cloud   │◄────────│   GitHub Releases│         │  IPFS (mirror)  │
...
└────────┬────────┘         └────────┬────────┘         └────────┬────────┘
         ...
         ▼                            ▼                            ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  JARVIS Hub (Raspberry Pi)                                             │
│                                                                         │
│  Update process (runs at 3 AM by default, configurable):               │
│  1. Download new image.tar to /tmp/jarvis-update/                       │
...
### Rollback Strategy
- Docker images are tagged with semantic version (`jarvis-core:v0.1.0`, `jarvis-core:v0.1.1`)
- Before update, current image is tagged as `jarvis-core:previous`
- Failed health check → `docker tag jarvis-core:previous jarvis-core:latest && docker restart jarvis-core`
...
- App notification: "JARVIS will update tonight at 3 AM. Tap to reschedule."
- Post-update notification: "JARVIS updated to v0.1.1. Tap to see what's new."
...
- Running HAOS alongside JARVIS on Pi would require dual-booting or VM
- **Our approach:** Run HA Core inside Docker on Pi OS — same functionality, simpler deployment
- HA runs as `homeassistant` container; JARVIS communicates via localhost API

### HAOS Update Strategy
- HA container updates independently from JARVIS core
- HA follows its own release cycle (bi-weekly)
- HA updates are applied in the same maintenance window
- JARVIS tests HA update compatibility before shipping JARVIS updates
...
Dockerfile:
  ...
  - JARVIS core (Rust) compiled to single binary
  - Python agent loaded as embedded interpreter (PyO3)
  - SQLite baked into image
  - Entrypoint: /usr/local/bin/jarvis-core
```
...
docker-compose.yml:
  services:
    jarvis-core:
      image: jarvis/core:${VERSION}
      volumes:
        - household-db:/var/lib/jarvis/
        - models:/usr/local/share/jarvis/models/
        - certs:/var/lib/jarvis/certs/
      ports:
        - "8443:8443"
...
1. Hub establishes outbound WebSocket to `wss://relay.jarvis.cloud`
...
- Certificate Common Name = `jarvis-<hub-id>.local`
...
- Emergency AP: "JARVIS-Setup" WiFi with captive portal for factory reset
...
- HA container adds ~1GB RAM overhead on top of JARVIS
