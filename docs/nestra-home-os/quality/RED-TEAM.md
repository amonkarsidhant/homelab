# Nestra Home OS — Red Team Exercise Plan v0.1
...
Red team exercises validate that Nestra's guardrails hold under adversarial conditions. Each exercise simulates a real-world attack scenario. The goal is not to "pass" — it is to discover where the guardrails fail before an adversary finds out for us.
...
| **Detection** | How Nestra or the SOC would detect this attack in progress |
...
1. **Alert fires** → Push notification to parent + all parent accounts: "Unusual activity detected on Nestra"
...
Demonstrate that a malicious or buggy skill installed on the Nestra hub cannot exfiltrate household behavioral data, voice recordings, device patterns, or any household information to an external party — regardless of what permissions it claims.
...
Step 2: Persistence Establishment
  ...
  - Skill modifies Nestra agent config to include its own outbound network rules
...
Demonstrate that a compromised Nestra cloud account (credential stuffing, phishing, or data breach) does — or does not — give an attacker direct control of the household's Nestra hub and its Matter devices.
...
Step 1: Account Compromise
  ...
  - OR: Phishing campaign targets Nestra users
  - OR: Nestra cloud infrastructure breach exposes password hashes

Step 2: Cloud Account Access
  - Attacker logs into Nestra cloud dashboard
...
## Scenario 4: Nestra Is Wrong — Incorrect Safety Action Trigger
...
Demonstrate that Nestra's pattern learning and inference engine cannot be manipulated — by adversarial input, sensor error, or emergent AI behavior — to incorrectly trigger a safety action (door unlock, alarm disable, camera disable) that puts the household at risk.
...
Step 1: Pattern Corruption via Sensor Manipulation
  ...
  - Nestra learns: "this space is always occupied after 10 PM"
  - Nestra inference: "no motion for 6 hours means home is empty" (false positive)
  - Nestra triggers away mode: locks doors (unintended — expected behavior)
  ...
  - Adversary exploits pattern learning: "when motion at front door and no one home, 
    do NOT automatically unlock" — Nestra learns a "do not unlock" exception
  - Later: legitimate "someone home" scenario is misclassified → family locked out
  - OR: inverse — adversarial manipulation makes Nestra think the home is "always empty"
  - → Nestra unlocks door for "expected arrival" when home is actually occupied
...
Step 3: Emergency False Positive (AI Hallucination Equivalent)
  - Nestra pattern learning engine encounters edge case: family away, smoke detector 
    triggers (false alarm), HVAC shuts off, all lights turn on (expected)
  - But Nestra has also learned: "smoke detector + away = security threat"
  - Nestra inference escalates: "active threat detected, unlock all doors for evacuation"
...
Step 5: Time-of-Check Time-of-Use (TOCTOU) on Safety State
  ...
  - Nestra inference: "all away → arm security mode → lock all doors"
  - Child sneaks back in through garage door (unlocked side door)
  - Motion detected inside → Nestra "reconsiders" → "someone is home after all"
...
1. **Safety action blocked by guardrail** → User receives immediate notification: "Nestra attempted to [action] — we blocked it and want to confirm: is this intended?"
...
Red team successfully causes Nestra to trigger a safety action (unlock door, disable camera, disable alarm) based on:
