# Nestra Home OS — Test Strategy v0.1
...
For a passive smart home hub, quality is measurable: device X responds in Y ms, protocol Z is implemented correctly. For Nestra — an ambient home intelligence that learns, reasons, and acts on behalf of the family — quality is more multidimensional:

1. **Reliability** — Nestra does what it says it will do, every time, without surprise
2. **Safety** — Nestra never does what it shouldn't, especially for safety-critical devices
3. **Privacy** — household data never leaves the home without informed, explicit consent
4. **Trustworthiness** — Nestra is honest about what it knows, what it inferred, and what it doesn't know
5. **Performance** — responses are fast enough to feel ambient, not like using a computer
6. **Usability** — non-technical homeowners can set up and use Nestra without help
...
Traditional software testing assumes: given the same input, you get the same output. Nestra violates this assumption — it learns, adapts, and changes behavior based on accumulated experience. A pattern learned today produces different actions tomorrow as context evolves.

### How We Test an Agent That Learns

| Challenge | Strategy |
|---|---|
| Test oracle problem — how do we know the "right" behavior? | Define behavioral contracts: Nestra may adapt, but it must NEVER violate its safety invariants. Tests verify invariants, not specific learned behaviors. |
...
The key insight: **test the boundaries Nestra must never cross, not the paths it takes within safe territory.**
...
| Safety actions require human confirmation | Nestra infers emergency; no confirmation triggered; verify safety action is BLOCKED | Safety action blocked until confirmed |
...
| System | auditd on Linux (Pi OS) for file opens | No read of user data directories by non-nestra processes |
...
Matter is the protocol layer that makes Nestra work with devices from multiple brands. Testing it requires both protocol-level correctness and end-to-end functional validation.

### Matter Testing Layers

| Layer | What | Tool/Method |
|---|---|---|
| Protocol compliance | Does Nestra correctly implement Matter spec (cluster commands, attributes, subscriptions)? | Matter Test Harness (CSA certified); Bluetooth sniffer for commissioning flow |
| Device pairing | Can Nestra successfully commission and pair with certified Matter devices? | Test with IKEA, Nanoleaf, TP-Link reference devices |
...
| Concurrent commands | Can Nestra handle 10 devices receiving commands simultaneously? | Concurrency test; verify no race conditions or fabric corruption |
| Thread/BLE boundary | Matter supports Thread and BLE commissioning. Does Nestra correctly handle both? | Test one device via Thread border router, one via BLE commissioning |
| Firmware updates | Matter OTA update process — does Nestra correctly push firmware to devices? | OTA test with a firmware-updatable device (e.g., smart plug) |
...
- [ ] Device pairs with Nestra hub in < 60 seconds
...
The Nestra mobile app is built in Flutter for iOS and Android. Testing strategy:
...
| Deep links | App Links (Android) / Universal Links (iOS) | Nestra deep links open correct screen; handle missing app gracefully |
