# Nestra Home OS — Threat Model v0.1
...
## The 10 Most Dangerous Things Nestra Could Do (If Unchallenged)

### 1. Unlock and Open Exterior Doors

**What:** Nestra processes a voice command or learned pattern to unlock the front door, then opens a smart lock.
...
**Attack path:** Child bypasses parental controls → issues "Nestra, unlock the front door" → smart lock opens. Or: malicious skill exfiltrates home layout + schedules → coordinated break-in.
...
**What:** Nestra silently disables all security cameras, motion sensors, and the alarm system — then continues to report "all systems secure" in the mobile app.
...
**What:** Nestra is manipulated into creating a permanent backdoor — a persistent guest account, a scheduled unlock, or a Matter device pairing that gives an attacker ongoing physical access.
...
**What:** A malicious or buggy skill turns the Nestra hub into an always-on listening device, exfiltrating household conversations to an external server.
...
**What:** Nestra is manipulated into repeatedly cycling HVAC on/off at extreme temperatures, or flickering smart lights at strobe frequencies, creating safety hazards (pipe freezing, seizures, fire).
...
**What:** Nestra learns when the family is home, asleep, away, eating, fighting (via voice tone analysis), sick — and this intimate behavioral profile is sold or stolen.
...
**What:** A compromised Nestra locks the family out of their own home — smart locks changed, garage locked, no voice commands honored — and demands Bitcoin to restore access.
...
**What:** Nestra's pattern learning incorrectly infers an emergency and triggers a safety action (unlocking doors, opening blinds, turning on all lights) at the wrong time — e.g., during a home invasion when silence is needed.

**Attack path:** Software bug or adversarial input → Nestra "infers" an emergency that doesn't exist → safety action creates real danger.
...
The Nestra hub sits on the home network alongside laptops, phones, and IoT devices. Any device compromise can potentially:

- Perform ARP spoofing to intercept Nestra ↔ cloud traffic (if cloud sync enabled)
...
**Worst case:** Compromised laptop → pivots to Nestra hub → gains persistent root access → controls all Matter devices, exfiltrates all local data.
...
- Malicious app impersonating Nestra (typosquatting, sideloading)
- Push notification spoofing (trigger false alarms)
- Deep link injection in the app

**Worst case:** Attacker installs malicious Nestra app from a third-party store → user logs in → attacker now has full hub control.
...
- Data breach at Nestra cloud infrastructure → full behavioral history exposed
- Rogue insider at Nestra or a cloud provider
...
| Time-based curfews | Asking Nestra to "remind me of my bedtime at 9pm" which pre-sets actions |
...
| Skill Ecosystem | Mass surveillance via malicious skills; Nestra brand permanently associated with privacy violations |
...
## The Single Most Dangerous Thing About Nestra

### The One Threat That Keeps Me Up at Night

**Unlock the front door via voice command without proper authorization.**

This is the apex threat because it is the convergence of Nestra's core value proposition (voice-first, ambient, acts on behalf of the family) with the most severe physical consequence (unauthorized physical entry).
...
2. **Nestra's core design** — Nestra is supposed to act on voice commands quickly and ambiently; introducing friction for door unlock defeats the UX goal
...
3. **The hub maintains a hardware-level mute** on the microphone array that cannot be overridden by software alone — there is always a physical or explicit digital confirmation required before Nestra processes any safety-critical command.
