# SUPPORT-OPS.md — Nestra Home OS Support Operations Model

**Version:** 0.1.0  
**Owner:** COO (nestra-coo)  
**Support Agent:** nestra-support-ops  
...
| **Ownership** | Support Ops Agent (nestra-support-ops) + human moderators |
| **Tools** | Intercom (in-app chat), Zendesk (tickets), community forum (forum.nestra.io) |

**T1 Handles:**
- "Nestra is not responding" — basic troubleshooting (restart app, check hub, check internet)
...
**T2 Handles:**
- Nestra hub not booting (Raspberry Pi image issues)
...
- Agent runtime failures (Nestra brain stops reasoning)
...
|---------|-------|-------|-----|
| **forum.nestra.io** | T1 + Community Agent | How-to questions, feature discussions, tips & tricks, community troubleshooting | Moderation within 4 hrs; answers not SLA-bound |
| **support@nestra.io** | T1 → T2 | Billing issues, account problems, technical bugs requiring agent logs | 4 hr first response (business hours) |
...
| **Security reports** | CQO directly | security@nestra.io — do NOT route through normal support | No SLA — CQO responds within 24 hrs |
...
**Knowledge Base URL:** docs.nestra.io / help.nestra.io
...
| **Troubleshooting** | "Nestra is not responding" flow, hub LED codes, app crash solutions, network requirements | T2 |
...
## 5. Mobile App In-App Support (When Nestra is Broken)

### Scenario: User Opens App and Nestra is Unresponsive

**If cloud sync is down (SEV1/2):**

1. **App shows degraded state banner:**
   > "Nestra is experiencing issues. Your hub is still running locally — [list what works offline]. Cloud features (remote access, cloud sync) are temporarily unavailable. [Track status →]"

2. **Tapping the banner** → links to status.nestra.io with live incident page
...
1. **App shows full-screen error state:**
   > "Hub Unreachable"
   > "Nestra hub at [address] is not responding. Please check: Is the hub powered on? Is your phone on the same network?"
