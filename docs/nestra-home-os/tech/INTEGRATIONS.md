...
Nestra Home OS must integrate with three external ecosystems: **Home Assistant** (local automation backbone), **voice assistants** (Google Home, Alexa, Apple HomeKit), and **decor brands** (OEM white-label API). This document defines those integration boundaries, protocols, and the brand OEM API surface.
...
Home Assistant is **not a competitor** — it is a force multiplier. Nestra uses Home Assistant as the integration layer for non-Matter devices (Zigbee, Z-Wave, Tuya, Shelly). We do not re-implement what HA does well.

| Capability | Nestra Owns | Home Assistant Owns |
|-----------|-------------|----------------------|
...
# Translates: HA entity → Nestra device model
```
...
**Events (HA → Nestra):**
| HA Event | Nestra Action |
|----------|--------------|
| `state_changed` | Update device state in Nestra SQLite, notify app |
...
| Nestra integration | HA custom component | localhost API |
...
We value **UX control and brand white-label** — HAOS is too opinionated. Running HA Core in Docker gives us HA's device integrations without surrendering the Nestra UX.
...
2. **Brand dilution:** If homeowners use "Hey Google" to control Nestra, Nestra becomes a backend — not the home intelligence platform. The brand relationship moves to Google.
...
|----------|-------------|
| **Local Assistant Bridge** | "Hey Nestra" is primary. Google Mini is an optional secondary. Homeowner can say "Hey Google, ask Nestra to..." — Nestra handles intent, Google handles ASR. |
| **Matter Bridge (Google Home)** | Nestra exposes devices to Google Home as a Matter controller. User can use Google Home app alongside Nestra app. This is a Matter-native integration, not an Action. |

**Phase 2 approach:** Expose Nestra-managed devices to Google Home via Matter. Google Home app can see and control Nestra devices. User can say "Hey Google, turn off living room lights" — command goes to Matter hub (Nestra), not Google Cloud.
...
### Future (Phase 2+)
- **Matter exposure:** Same as Google Home — Nestra is a Matter hub, exposes devices via Matter to Alexa
- **Alexa as optional wake word:** Alexa Echo devices can act as optional secondary microphone, routing to Nestra local inference via LAN
...
**Our approach:**
- Nestra does NOT implement HomeKit Accessory Protocol (HAP) — that's Apple's certification burden
- Matter devices managed by Nestra appear in Apple Home via Matter (no separate pairing needed)
- User uses Apple Home app OR Nestra app — their choice
...
Decor brands (IKEA, Wayfair, Ashley) white-label Nestra as "their" smart home brain. The brand OEM API allows:

1. Brand app to connect to Nestra hub and control branded devices
2. Brand to customize Nestra voice name, wake word, personality
...
**Purpose:** Allows brand to embed Nestra control in their existing app

```dart
// Brand app initialization
final nestra = NestraOEM(
  hubId: 'BRICK-XXXX',       // Brand-prefixed hub ID
  brandId: 'ikea',            // Brand identifier
  apiKey: 'brand_xxx',        // Per-brand API key (Nestra Cloud issues)
  oemEndpoint: 'ikea.nestra.cloud', // Brand's Nestra Cloud tenant
);

// Connect to household hub
await nestra.connect(
  householdId: 'hh_abc123',
  oemToken: 'tok_xxx',        // OAuth token from brand's identity
);

// Issue commands
await nestra.devices.setState(
  deviceId: 'matter_001',
  state: { 'on_off': true, 'brightness': 80 },
);

// Receive events
nestra.devices.events.listen((event) {
...
|--------------|-------|----------------|
| Voice name | "Hey IKEA" vs "Hey Nestra" | Wake word model swap + TTS voice model |
...
Skills extend Nestra capabilities (e.g., "order more paper towels when low", "report energy usage to utility"). Skills are:
- Distributed via Nestra Skill Store (brand-curated or Nestra-curated)
- Sandboxed Python subprocesses with permission system
- Reviewed by Nestra CQO team before listing
...
- Two update mechanisms (Nestra + HA) require coordination
