...
```
┌─────────────────┐    Unix Domain Socket    ┌─────────────────┐
│  Nestra Core    │ ◄───── JSON RPC ────────► │  Nestra Agent   │
│  (Rust daemon)  │                           │  (Python)       │
│  - Matter       │                           │  - Intent       │
...
### Docker on Pi
- **Why Docker:** Nestra ships as a Docker image that runs on Pi OS. This ensures consistent environment and clean updates.
...
- `nestra-core`: Rust Matter + voice
- `nestra-agent`: Python intent + patterns
- `nestra-ha`: Home Assistant OS (official image)
- `nestra-mqtt`: Mosquitto (for HA ↔ Nestra events)
...
**Nestra Cloud = Cloudflare Workers + Durable Objects**
...
- Single file backup (cp /var/lib/nestra/household.db)
...
- Encrypted household state snapshots (E2E encrypted, Nestra Cloud cannot read)
...
```
Nestra Hub (Raspberry Pi 4/5)
├── Raspberry Pi OS 64-bit (Bookworm)
│
├── Docker Engine
│   ├── nestra-core (Rust: Matter, voice, API, device state)
